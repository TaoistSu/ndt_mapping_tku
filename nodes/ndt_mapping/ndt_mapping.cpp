...
static void output_callback(const autoware_config_msgs::ConfigNDTMappingOutput::ConstPtr& input)
{
  ...
  else
  {
    ...
    std::cout << "Saved " << map_filtered->points.size() << " filtered data points to " << filename << "." << std::endl;
   ...
   copyPointCloud (*map_filtered , *keypoints_ptr);
   pcl::PointCloud<pcl::PointXYZ>::iterator index = keypoints_ptr->begin();
   for(index;index!=keypoints_ptr->end()-2;++index)
   	keypoints_ptr->erase(index);//删除第一个

   pcl::io::savePCDFileASCII<pcl::PointXYZ>("/home/light/test/autoware-200330-10.pcd", *keypoints_ptr);
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (initial_scan_loaded == 0)
  {
    ...
    //把transformed_scan_ptr先滤波再加入地图减小点数
  ...
  std::cout << "Use filter: Ture " << std::endl;
  std::cout << "-----------------------------------------------------------------" << std::endl;
