...
double gnss_ori_x = 0.0;
double gnss_ori_y = 0.0;
double gnss_ori_z = 0.0;
double gnss_rota_x = 0.0;
double gnss_rota_y = 0.0;
double gnss_rota_w = 0.0;
double gnss_rota_z = 0.0;
double roll=0.0, pitch=0.0, yaw=0.0;//定义存储r\p\y的容器
...
{
	gnss_ori_x = input->pose.position.x;
	gnss_ori_y = input->pose.position.y;
	gnss_ori_z = input->pose.position.z;
...
void save_nd_map(const char *name)
{
  ...
  std::cout<<"Saved Map to :" <<name<<std::endl;
...
{
  save_nd_map((const char *)"/home/light/test/changfang_tku.pcd");
  over_flag = 1;
}
...
int main(int argc, char *argv[])
{
  ...
  if (nh.getParam("tf_x", _tf_x))//读到是1,取反肯定0啊
  {
  ...
  if (nh.hasParam("map_name"))
  {
    std::cout << "map_name is not set." << std::endl;
    return 1;
  }
  std::cout << "g_ini_x: " << g_ini_x<<std::endl;
  std::cout << "g_ini_y: " << g_ini_x<<std::endl;
  std::cout << "g_ini_z: " << g_ini_z<<std::endl;
  std::cout << "g_ini_roll: " << g_ini_roll<<std::endl;
  std::cout << "g_ini_pitch: " << g_ini_pitch<<std::endl;
  std::cout << "g_ini_yaw: " << g_ini_yaw<<std::endl;
  std::cout << "init_yaw: " << g_ini_yaw<<std::endl;
  ...
  std::cout << "g_use_gnss: " << g_use_gnss<<std::endl;
  std::cout << "map_name: " << _mapname<<std::endl;
  //printf("map_name:%s\n",_mapname);
  ...
  ros::Subscriber sub = nh.subscribe("saveMapCommand", 1000, chatterCallback);
  ros::Subscriber pose_sub = nh.subscribe("/gnss_pose", 100, PoseCallback);//predict_pose
  ///ros::spin();
  //.....
  while(ros::ok()&&(!over_flag))
  {
    /*if (g_use_gnss)
    {
        try
        {
          listener.lookupTransform("world", "gps", ros::Time(0), gps_tf_on_world);
	  gnss_ori_x = gps_tf_on_world.getOrigin().x();
	  gnss_ori_y = gps_tf_on_world.getOrigin().y();
	  gnss_ori_z = gps_tf_on_world.getOrigin().z();
	  gnss_rota_x = gps_tf_on_world.getRotation().x();
	  gnss_rota_y = gps_tf_on_world.getRotation().y();
	  gnss_rota_z = gps_tf_on_world.getRotation().z();
	  gnss_rota_w = gps_tf_on_world.getRotation().w();
        }
        catch (tf::TransformException ex)
        {
          ROS_ERROR("%s", ex.what());
  ...
  //save_nd_map((char *)"/home/light/test/changfang_tku.pcd");
  save_nd_map(_mapname.c_str());
  std::cout<<"ndt over!"<<std::endl;
  return 1;
}
